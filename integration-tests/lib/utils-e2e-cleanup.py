...
Required env:
  GITHUB_TOKEN
  TEMP_REPO_NAME  (full org/repo of the temporary fork, e.g. hacbs-release-tests/catalog-utils-e2e-<uid>)

Optional env:
  CATALOG_BASE_SHA
  CATALOG_REPO     (default: konflux-ci/release-service-catalog)
  CATALOG_REF      (default: development)
  INTEGRATION_TESTS_SCRIPTS_DIR  If set and contains delete-repository.sh, skip cloning catalog.
"""
...
def _warn_catalog_drift(
    *, catalog_repo: str, catalog_ref: str, catalog_base_sha: str
) -> None:
    ...
    if current and current != catalog_base_sha:
        ...
        print("  E2E ran against the older catalog snapshot. Re-run if you need latest catalog.")
        print()


def _resolve_scripts_dir(
    *, github_token: str, catalog_repo: str, catalog_ref: str
) -> tuple[Path, Path | None]:
    """Directory containing ``delete-repository.sh``, and temp clone root to rm -rf (or None)."""
    ...
    td = Path(tempfile.mkdtemp(prefix="utils-e2e-cleanup-"))
    clone_dest = td / "catalog"
    url = f"https://{github_token}@github.com/{catalog_repo}.git"
    try:
        subprocess.run(
            [
                "git",
    ...
    if not (scripts / "delete-repository.sh").is_file():
        shutil.rmtree(td, ignore_errors=True)
        print(
            f"ERROR: delete-repository.sh missing after clone: {scripts / 'delete-repository.sh'}",
            file=sys.stderr,
        )
        sys.exit(1)
    return scripts, td
...
def main() -> None:
    ...
    temp_repo_name = _require_env("TEMP_REPO_NAME")
    ...
    scripts_dir, clone_root = _resolve_scripts_dir(
