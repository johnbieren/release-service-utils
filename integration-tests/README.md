...
1. **Cluster:** `kubectl`/`oc` context targets the right cluster. **`NAMESPACE`** must exist (default `rhtap-release-2-tenant`). In that namespace, ensure **`githubTokenSecretName`** (default `e2e-test-github-token`) exists for clone/push/`finally`. In **`rhtap-release-2-tenant`**, ensure the three e2e Secrets exist (defaults `e2e-test-vault-password`, `e2e-test-github-token`, `e2e-test-service-account-kubeconfig`) for the **child** catalog run, or override the corresponding env vars when invoking **`run-test.sh`**.
