...
def test_publish_repositories_publish_on_push_false_skips_patch(tmp_path: Path) -> None:
    """Skip PATCH and catalog URL when publish_on_push is false."""
    sign_file = tmp_path / "sign-registry-access.txt"
    sign_file.write_text("", encoding="utf-8")
...
def test_publish_unknown_catalog_prefix_raises(tmp_path: Path) -> None:
    """Fail when a repository URL has an unsupported Quay prefix."""
    sign_file = tmp_path / "sign-registry-access.txt"
    sign_file.write_text("", encoding="utf-8")
    patch_called = False

    def fail_patch(*_a: Any, **_k: Any) -> None:
        nonlocal patch_called
        patch_called = True
        pytest.fail("patch must not run before catalog URL validation")
...
def test_main_unexpected_failure_exits_one(monkeypatch: pytest.MonkeyPatch) -> None:
    """Exit one when an unexpected exception escapes the workflow."""
...
def test_main_failure_exits_one(monkeypatch: pytest.MonkeyPatch) -> None:
    """Exit one with a clear message when the workflow raises."""
    monkeypatch.setenv("PARAM_DATA_DIR", "/tmp")
    monkeypatch.setenv("PARAM_SNAPSHOT_PATH", "snap.json")
    monkeypatch.setenv("PARAM_DATA_PATH", "data.json")
    monkeypatch.setenv("PARAM_RESULTS_DIR_PATH", "results")
    ...
    with mock.patch(
        ...
        with pytest.raises(SystemExit) as exc:
            publish_pyxis_repository.main()
    assert "No valid snapshot file" in str(exc.value)
