...
**Read this first:** Unlike internal OSIDB tasks, this script **fails the Tekton step** on error — [`main`](https://github.com/konflux-ci/release-service-utils/blob/main/scripts/python/tasks/managed/publish_pyxis_repository.py#L257-L279) raises `SystemExit` with a `publish_pyxis_repository.py: …` message (or `must be set` from missing env). There is no `result: Success` file.
...
| [`rh-advisories`](https://github.com/konflux-ci/release-service-catalog/blob/development/pipelines/managed/rh-advisories/rh-advisories.yaml) | **Skipped** when `filter-already-released-advisory-images` sets `skip_release=true` ([L692-L696](https://github.com/konflux-ci/release-service-catalog/blob/development/pipelines/managed/rh-advisories/rh-advisories.yaml#L692-L696)) |
...
| Pyxis login | Kubernetes secret mounted at `PYXIS_SECRET_MOUNT` (default `/etc/secrets`) — two files: `cert` and `key`. The script passes both to every Pyxis HTTP call ([`cert` tuple](https://github.com/konflux-ci/release-service-utils/blob/main/scripts/python/tasks/managed/publish_pyxis_repository.py#L229-L231)). |
| Pyxis environment | `PARAM_SERVER` → API base URL ([`pyxis_api_url_for_server`](https://github.com/konflux-ci/release-service-utils/blob/main/scripts/python/helpers/pyxis_api.py#L40-L45)); optional test override `PYXIS_URL` |

### Quay URL → Pyxis mapping

| Quay prefix | Pyxis registry | Repository name |
|-------------|----------------|-----------------|
| `quay.io/redhat-prod/…`, `quay.io/redhat-pending/…` | `registry.access.redhat.com` | last path segment with `----` → `/` ([`pyxis_repository_from_quay_url`](https://github.com/konflux-ci/release-service-utils/blob/main/scripts/python/helpers/pyxis_api.py#L55-L58)) |
...
| Tekton result | `signRegistryAccessPath` — **relative** path to that txt file from `dataDir` ([L206-L211](https://github.com/konflux-ci/release-service-utils/blob/main/scripts/python/tasks/managed/publish_pyxis_repository.py#L206-L211)) |
...
1. **GET** Pyxis repository JSON ([`get_repository_json`](https://github.com/konflux-ci/release-service-utils/blob/main/scripts/python/helpers/pyxis_api.py#L93-L113)) — needs `_id`.
2. **Sign list:** if registry is `registry.access.redhat.com` and Pyxis `requires_terms` is **`false`** (JSON boolean), append `pyxis_repo` to `sign-registry-access.txt` — see [`should_add_sign_registry_access`](https://github.com/konflux-ci/release-service-utils/blob/main/scripts/python/tasks/managed/publish_pyxis_repository.py#L50-L55). (Catalog YAML text says `requires_terms=true`; the **code** uses `false`. Flatpak registries never qualify.)
3. **PATCH** only when:
   - `data.pyxis.skipRepoPublishing` is not true ([`skip_repo_publishing`](https://github.com/konflux-ci/release-service-utils/blob/main/scripts/python/tasks/managed/publish_pyxis_repository.py#L34-L39)), **and**
   - Pyxis `publish_on_push` is **exactly** `true` ([L145-L153](https://github.com/konflux-ci/release-service-utils/blob/main/scripts/python/tasks/managed/publish_pyxis_repository.py#L145-L153)).
4. PATCH body: `published: true`, plus `source_container_image_enabled: true` when [`component_push_source_container`](https://github.com/konflux-ci/release-service-utils/blob/main/scripts/python/helpers/snapshot.py#L22-L30) is true (default from `mapping.defaults.pushSourceContainer`, default **true**).
...
### [`main`](https://github.com/konflux-ci/release-service-utils/blob/main/scripts/python/tasks/managed/publish_pyxis_repository.py#L257-L279)

Loads required env and paths, calls [`run_publish_pyxis_repository`](https://github.com/konflux-ci/release-service-utils/blob/main/scripts/python/tasks/managed/publish_pyxis_repository.py#L182-L254). On `FileNotFoundError`, `ValueError`, `OSError`, or any other exception → `SystemExit(f"{PROG}: {exc}")`. Success → return `0`.

### [`run_publish_pyxis_repository`](https://github.com/konflux-ci/release-service-utils/blob/main/scripts/python/tasks/managed/publish_pyxis_repository.py#L182-L254)
...
3. Logs start, Pyxis URL ([`resolve_pyxis_api_url`](https://github.com/konflux-ci/release-service-utils/blob/main/scripts/python/tasks/managed/publish_pyxis_repository.py#L22-L31)), skip flag, default `pushSourceContainer`.
4. Calls [`publish_repositories`](https://github.com/konflux-ci/release-service-utils/blob/main/scripts/python/tasks/managed/publish_pyxis_repository.py#L58-L179).
5. Writes [`publish-pyxis-repository-results.json`](https://github.com/konflux-ci/release-service-utils/blob/main/scripts/python/tasks/managed/publish_pyxis_repository.py#L18).
6. Logs completion with published count.

### [`publish_repositories`](https://github.com/konflux-ci/release-service-utils/blob/main/scripts/python/tasks/managed/publish_pyxis_repository.py#L58-L179)
...
- Maybe appends sign-list line; maybe PATCH and append to `catalog_urls` via [`catalog_url_for_repository`](https://github.com/konflux-ci/release-service-utils/blob/main/scripts/python/helpers/pyxis_api.py#L71-L78).

### Small helpers

| Function | Role |
|----------|------|
| [`build_publish_payload`](https://github.com/konflux-ci/release-service-utils/blob/main/scripts/python/tasks/managed/publish_pyxis_repository.py#L42-L47) | PATCH JSON |
| [`resolve_pyxis_api_url`](https://github.com/konflux-ci/release-service-utils/blob/main/scripts/python/tasks/managed/publish_pyxis_repository.py#L22-L31) | `PYXIS_URL` or `PARAM_SERVER` map |
| [`skip_repo_publishing`](https://github.com/konflux-ci/release-service-utils/blob/main/scripts/python/tasks/managed/publish_pyxis_repository.py#L34-L39) | Reads `pyxis.skipRepoPublishing` |

Pyxis HTTP: GET uses 120s timeout and returns error bodies ([`allow_error_status=True`](https://github.com/konflux-ci/release-service-utils/blob/main/scripts/python/helpers/pyxis_api.py#L103-L107)); PATCH raises on non-2xx ([L134-L139](https://github.com/konflux-ci/release-service-utils/blob/main/scripts/python/helpers/pyxis_api.py#L134-L139)).
...
|---------------|------|--------------|---------------|
| *(no `INFO:` lines)* | Failed | Missing env before work starts | stderr for `PARAM_* must be set` or `RESULT_SIGN_REGISTRY_ACCESS_PATH must be set`; [catalog env L167-L183](https://github.com/konflux-ci/release-service-catalog/blob/development/tasks/managed/publish-pyxis-repository/publish-pyxis-repository.yaml#L167-L183) |
